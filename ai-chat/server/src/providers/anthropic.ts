...
export async function* streamAnthropic(
  ...
  const systemMessages = messages.filter(m => m.role === 'system').map(m => ({ type: 'text' as const, text: m.content }));
  ...
  const stream = client.messages.stream({
    model: options.model, max_tokens: options.maxTokens ?? 4096,
    temperature: options.temperature ?? 0.7, system: systemPrompt, messages: conversationMessages,
  });
