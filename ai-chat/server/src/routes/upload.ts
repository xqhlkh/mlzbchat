...
router.post('/file', upload.array('files', 10), (req: Request, res: Response) => {
  ...
  try {
    const results = files.map(file => {
      ...
      const textContent = isImage ? '' : extractTextContent(file.path, file.mimetype);
      ...
      if (isImage) {
        result.imageBase64 = encodeImageBase64(file.path);
        result.content = `[图片文件: ${file.originalname}]`;
      }
